  ...
  queryText.executeRevEngQuick = function() {
    ...
    promise.then(function(val){
      ...
      if(queryText.learnedQuery === '') {
        ...
        relaxed += '?x <http://www.w3.org/2000/01/rdf-schema#label> ?label . FILTER(LANGMATCHES(LANG(?label), "en")) { select ?x (MIN(STR(?type)) as ?minType) where { ?x <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> ?type } group by ?x } } ORDER BY DESC ( <LONG::IRI_RANK> (?x) ) limit 10';
